...
def split_infobox():
    ...
    *.box.pos is the position counted from the begining of a field
    """
    bwfile = ["processed_data_disorder/train/train.box.val", 
              "processed_data_disorder/valid/valid.box.val", 
              "processed_data_disorder/test/test.box.val"]
    bffile = ["processed_data_disorder/train/train.box.lab", 
              "processed_data_disorder/valid/valid.box.lab", 
              "processed_data_disorder/test/test.box.lab"]
    bpfile = ["processed_data_disorder/train/train.box.pos", 
              "processed_data_disorder/valid/valid.box.pos", 
              "processed_data_disorder/test/test.box.pos"]
    ...
    for fboxes in boxes:
        ...
        for ib in box:
            ...
            assert(len(box_single_label) == len(box_single_pos))
            last = None
            start = 0
            starts = []
            lens = []
            for i, f in enumerate(box_single_label):
                if last == None or f == last:
                    last = f    
                    continue
                else:
                    if box_single_pos[i] > box_single_pos[i-1]:
                        wrong_count += 1
                        # print(box_single_pos[i-1], box_single_pos[i])
                        # print(box_single_label[i-1], box_single_label[i])
                    ...
                    last = f
            starts.append(start)
            lens.append(len(box_single_label) - start)
            ...
            assert len(s_indices) == len(box_single_word)
            box_single_word = np.array(box_single_word)[s_indices].tolist()
            box_single_label = np.array(box_single_label)[s_indices].tolist()
            box_single_pos = np.array(box_single_pos)[s_indices].tolist()
...
def reverse_pos():
    ...
    bpfile = ["processed_data_disorder/train/train.box.pos", "processed_data_disorder/valid/valid.box.pos", "processed_data_disorder/test/test.box.pos"]
    bwfile = ["processed_data_disorder/train/train.box.rpos", "processed_data_disorder/valid/valid.box.rpos", "processed_data_disorder/test/test.box.rpos"]
...
def check_generated_box():
    ftrain = ["processed_data_disorder/train/train.box.val",
              "processed_data_disorder/train/train.box.lab",
              "processed_data_disorder/train/train.box.pos",
              "processed_data_disorder/train/train.box.rpos"]
    ftest  = ["processed_data_disorder/test/test.box.val", 
              "processed_data_disorder/test/test.box.lab",
              "processed_data_disorder/test/test.box.pos",
              "processed_data_disorder/test/test.box.rpos"]
    fvalid = ["processed_data_disorder/valid/valid.box.val", 
              "processed_data_disorder/valid/valid.box.lab", 
              "processed_data_disorder/valid/valid.box.pos",
              "processed_data_disorder/valid/valid.box.rpos"]
...
def split_summary_for_rouge():
    ...
    bwfile = ["processed_data_disorder/test/test_split_for_rouge/", "processed_data_disorder/valid/valid_split_for_rouge/"]
...
def table2id():
    fvals = ['processed_data_disorder/train/train.box.val',
             'processed_data_disorder/test/test.box.val',
             'processed_data_disorder/valid/valid.box.val']
    flabs = ['processed_data_disorder/train/train.box.lab',
             'processed_data_disorder/test/test.box.lab',
             'processed_data_disorder/valid/valid.box.lab']
    fsums = ['original_data/train.summary',
             'original_data/test.summary',
             'original_data/valid.summary']
    fvals2id = ['processed_data_disorder/train/train.box.val.id',
                'processed_data_disorder/test/test.box.val.id',
                'processed_data_disorder/valid/valid.box.val.id']
    flabs2id = ['processed_data_disorder/train/train.box.lab.id',
                'processed_data_disorder/test/test.box.lab.id',
                'processed_data_disorder/valid/valid.box.lab.id']
    fsums2id = ['processed_data_disorder/train/train.summary.id',
                'processed_data_disorder/test/test.summary.id',
                'processed_data_disorder/valid/valid.summary.id']
    vocab = Vocab()
...
def make_dirs():
    ...
    if not os.path.exists('processed_data_disorder'):
        os.mkdir("processed_data_disorder/")
        os.mkdir("processed_data_disorder/train/")
        os.mkdir("processed_data_disorder/test/")
        os.mkdir("processed_data_disorder/valid/")
        os.mkdir("processed_data_disorder/test/test_split_for_rouge/")
        os.mkdir("processed_data_disorder/valid/valid_split_for_rouge/")
