...
parser.add_argument("--plot_attention", type=bool, default=False)
...
def train(dataloader, model):
    ...
    for _ in range(args.epoch):
        for x in dataloader.batch_iter(trainset, args.batch_size, True):
            # print(x['dec_in'].__class__)
            # print(len(x['dec_in'][0]))
            # print(len(x['dec_in'][1]))
            # print(x['dec_in'])
            model.train()
            lloss = model(x)
...
def generate_attention_plot(load_dir, normalize_on_decout_axis=True):
    ...
    for word in np.array(x['enc_in']).reshape(-1):
        enc_in.append(v.id2word(word))
    for word in np.array(x['enc_fd']).reshape(-1):
        enc_fd.append(v.id2key(word))
...
if __name__=='__main__':
    jt.flags.use_cuda = 1
    # generate_attention_plot('results/res/1639620199071/loads/30/model')
    main()
